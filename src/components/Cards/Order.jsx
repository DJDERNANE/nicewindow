...
export const OrderCard = ({
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() =>
        display === "carpentry"
          ? navigation.navigate("CRPNTRYProfileOrder", { order_id: id })
          : navigation.navigate("PSOrder", { order_id: id })
      }
    >
      <View style={styles.clientNameView}>
        <TextBold style={styles.clientName}>{client_name}</TextBold>
        <Badge color={setOrderStatus(status).color}>
          {setOrderStatus(status).label}
        </Badge>
      </View>

      <TextMedium style={styles.detailsText}>
        {date.split("T")[0]} | Prix: {price} DZD
      </TextMedium>
    </TouchableOpacity>
  );
};

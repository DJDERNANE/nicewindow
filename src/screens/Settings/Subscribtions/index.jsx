...
class SubscribtionsSettingsScreen extends Component {
  ...
  async storeImage() {
    ...
    if(user !== null && this.state.selectedImage)
    {
      ...
      formData.append('proof', { uri: this.state.selectedImage, name: filename, type });
    ...
    return(
      <Screen>
              ...
              <View style={styles.card}>
                <View style={styles.cardHeader}>
                  <TextExtraBold style={styles.packageNameText}>
                    {i18n.language === 'ar' ? s.subscribtionData.package?.name_ar.toUpperCase() : s.subscribtionData.package?.name_en.toUpperCase()}
                  </TextExtraBold>
                  {s.subscribtionData.status === 0 ?
                    <Badge color={WARNING}>{ i18n.t('status.subscribtions.waiting') }</Badge>
                  :
                  s.subscribtionData.status === 1 ?
                    <Badge color={SUCCESS}>{ i18n.t('status.subscribtions.active') }</Badge>
                  :
                    <Badge color={DANGER}>{ i18n.t('status.subscribtions.expired') }</Badge>
                  }
                </View>
                <View style={styles.cardBody}>
                  <TextBold style={styles.cardBodyText}>{ i18n.t('settings.subscribtions.start') }: <TextMedium>{s.subscribtionData.start}</TextMedium></TextBold>
                  <TextBold style={styles.cardBodyText}>{ i18n.t('settings.subscribtions.end') }: <TextMedium>{s.subscribtionData.end}</TextMedium></TextBold>
                </View>
                {s.subscribtionData.status > 1 &&
        ...
        <UpdateSubscribtionModal 
          ...
          onPackageChange={(package_id) => this.setState({selectedPackage: package_id})}
        />
      </Screen>
    );
  }

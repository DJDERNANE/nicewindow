...
class CarpentryRevenuScreen extends Component {
  ...
  getRevenus = async () => {
    try {
      ...
      const response = await fetch(
        `${CARPENTRY_API_LINK}/accounting/totalrevenuDetails`,
        {
          method: "GET",
          headers: {
  ...
  getDetails = async () => {
    ...
    try {
      const user = await AsyncStorage.getItem("user");
      const response = await fetch(
        `${CARPENTRY_API_LINK}/accounting/revenu/details`,
        {
          method: "GET",
          headers: {
            Accept: "application/json",
        ...
        <ScrollView>
          <View style={styles.container}>
            <TouchableOpacity style={styles.accountingCard}>
              <View>
                <TextBold style={[styles.sectionTitleText, { color: SUCCESS }]}>
                  مدفوع
                </TextBold>
                <TextExtraBold
            ...
            <TouchableOpacity style={styles.accountingCard}>
              <View>
                <TextBold style={[styles.sectionTitleText, { color: WARNING }]}>
                  كريدي
                </TextBold>
                <TextExtraBold
                  style={[styles.sectionContentText, { color: WARNING }]}
                >
                  {s.credit}.00 DZD
                </TextExtraBold>
              </View>

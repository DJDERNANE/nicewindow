...
class RevenuScreen extends Component {
  ...
  getRevenus = async () => {
    try {
      ...
      const response = await fetch(`${SUPPLIER_API_LINK}/accounting/totalrevenuDetails`, {
        method: 'GET',
        headers: {
  ...
  getDetails = async () => {
    ...
    try {
      const user = await AsyncStorage.getItem('user');
      const response = await fetch(`${SUPPLIER_API_LINK}/accounting/revenu/details`, {
        method: 'GET',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
    ...
    return (
      <View style={{ backgroundColor: '#e4e4e4', marginVertical: 10, padding: 10 }}>
        <View style={styles.header}>
          <View>
            <TextBold>{item.carpentry.firstname} {item.carpentry.lasttname} </TextBold>

...
const CarpentryAccountingScreen = () => {
  ...
  const getDetails = async () => {
    try {
      ...
      const response = await fetch(
        `${SUPPLIER_API_LINK}/accounting/spend/details`,
        {
          method: "GET",
          headers: {
            Accept: "application/json",
  ...
  const getPurchase = async () => {
    try {
      const user = await AsyncStorage.getItem("user");
      const response = await fetch(
        `${CARPENTRY_API_LINK}/carpentry/purchases`,
        {
          method: "GET",
          headers: {
            Accept: "application/json",
  ...
  const SpendCard = ({ item }) => {
    ...
    return (
      <View
        style={{ backgroundColor: "#e4e4e4", marginVertical: 10, padding: 10 }}
      >
  ...
  const filter = async () => {
    ...
    try {
      const user = await AsyncStorage.getItem("user");
      const url = endDay
        ? `${CARPENTRY_API_LINK}/accounting/total/filter?startday=${startDay}&&endday=${endDay}`
        : `${CARPENTRY_API_LINK}/accounting/total/filter?startday=${startDay}`;
          ...
          {loading ? (
            ...
            <View>
              ...
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate("CarpentryRevenu")}
              >
                <View>
                  <TextBold
                    style={[styles.sectionTitleText, { color: SUCCESS }]}
                  >
                    {i18n.t("profit.profit")}
                  </TextBold>
                  <TextExtraBold
                    style={[styles.sectionContentText, { color: SUCCESS }]}
                  >
                    {revenu}.00 DZD
                  </TextExtraBold>
                </View>
              ...
              <TouchableOpacity style={styles.accountingCard}>
                <View>
                  <TextBold
                    style={[styles.sectionTitleText, { color: DANGER }]}
                  >
                    {i18n.t("profit.products")}
                  </TextBold>
                  <TextExtraBold
                    style={[styles.sectionContentText, { color: DANGER }]}
                  >
                    {purchase}.00 DZD
                  </TextExtraBold>
                </View>

...
import { SUPPLIER_API_LINK, CARPENTRY_API_LINK } from "../../../utils/constants";
...
const CarpentryAccountingScreen = () => {
  ...
  useEffect(() => {
    ...
    setTotal(revenu - spend - purchase);
  }, [revenu, spend, purchase]);
  ...
  const getPurchase = async () => {
    try {
      const user = await AsyncStorage.getItem('user');
      const response = await fetch(`${CARPENTRY_API_LINK}/carpentry/purchases`, {
        method: 'GET',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
  ...
  const getRevenu = async () => {
    try {
      const user = await AsyncStorage.getItem('user');
      const response = await fetch(`${CARPENTRY_API_LINK}/carpentry/revenu`, {
        method: 'GET',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
  ...
  const filter = async() => {
    ...
    try {
      const user = await AsyncStorage.getItem('user');
      const url = endDay? `${CARPENTRY_API_LINK}/accounting/total/filter?startday=${startDay}&&endday=${endDay}` : `${CARPENTRY_API_LINK}/accounting/total/filter?startday=${startDay}`
        ...
        <View style={styles.container}>
          {loading ? (
            ...
            <View>
              ...
              <TouchableOpacity style={styles.accountingCard} onPress={() => navigation.navigate('CarpentryRevenu')}>
                <View>
              ...
              <TouchableOpacity style={styles.accountingCard}>
                <View>
                  <TextBold style={[styles.sectionTitleText, { color: DANGER }]}>{ i18n.t('profit.products') }</TextBold>
                  <TextExtraBold style={[styles.sectionContentText, { color: DANGER }]}>{purchase}.00 DZD</TextExtraBold>
                </View>
              </TouchableOpacity>
              <TouchableOpacity style={styles.accountingCard} onPress={handleSpendDetails}>
                <View>
                  <TextBold style={[styles.sectionTitleText, { color: DANGER }]}>{ i18n.t('profit.spend') }</TextBold>
...
export default CarpentryAccountingScreen;

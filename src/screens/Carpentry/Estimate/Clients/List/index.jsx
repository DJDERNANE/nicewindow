      ...
      <Screen>
        <ScrollView refreshControl={
          ...
          <View style={styles.container}>
            ...
            <TextBold style={{fontSize: 18, marginBottom: 6}}>{ i18n.t('clients.clients') }</TextBold>
            {s.clientsLoading ?
            ...
            :
              s.searchResults && s.searchResults.length > 0 ?
                s.searchResults.map((data, key) => {
                  return(
                    <TouchableOpacity style={styles.clientView} key={key} onPress={() => this.props.navigation.navigate('ClientOrder', {client: data})}>
                      <TextBold style={styles.clientNameText}>{data.name}</TextBold>
                    </TouchableOpacity>
                  );
                })
              :

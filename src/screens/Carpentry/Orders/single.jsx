...
class CarpentryProfileOrderScreen extends Component {
  ...
  async getOrder() {
    ...
    fetch(
      CARPENTRY_API_LINK + "/profile/order/" + this.props.route.params.order_id,
      {
        method: "GET",
        headers: {
              ...
              <View>
                <View style={styles.orderItem}>
                  <TextMedium style={styles.orderItemText}>
                    ...
                    {s.orderData.supplier.firstname +
                      " " +
                      s.orderData.supplier.lastname}
                  </TextMedium>
                </View>
                {s.orderData.profile_order_products &&
                  s.orderData.profile_order_products.length > 0 &&
                  s.orderData.profile_order_products.map((data, key) => {
                    return (
                      <View
                        style={[styles.orderItem, styles.orderProfileView]}
                        key={key}
                      >
                        <View>
                          <Image
                            source={{ uri: MAIN_LINK + data.profile.icon }}
                ...
                <View style={styles.orderItem}>
                  <TextMedium style={styles.orderItemText}>
                    ...
                    <Badge color={setOrderStatus(s.orderData.status).color}>
                      {setOrderStatus(s.orderData.status).label}
                    </Badge>
                  </TextMedium>
                </View>
              </View>
        ...
        {s.orderData && (
          <View style={styles.footer}>
            <ButtonDefault
              ...
              onPress={() =>
                Linking.openURL("tel:" + s.orderData.supplier.phone_number)
              }
              size={"md"}
            />
          </View>
